...
import { DynatraceEnvironmentAPIV2, DynatraceTenantAPI } from "@dt-esa/dynatrace-api-client";
...
exports.spring4ShellProd = async (req, res) => {
    ...
    .filter(process => 
        (process.properties.softwareTechnologies as Array<{type: string, edition: string, version: string }>)
            .filter(st => st.type == 'JAVA') // filter technologies to java
            .filter(st =>
                /^9\.\d+\.\d+/.test(st.version)  || 
                ...
                /^18\.\d+\.\d+/.test(st.version) 
                ) // Filter to all that are a potentially vulnerable version
            .length > 0
    )
    ...
    const softwareTechnologiesFlattened = removedRelationships.map(process => {
        const tech = process.properties.softwareTechnologies as Array<{type: string, edition: string, version: string}>;
        let fixed = process; 
